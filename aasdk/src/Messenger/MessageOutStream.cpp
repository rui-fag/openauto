...
namespace
{
std::string hexDump(const f1x::aasdk::common::Data& data)
{
...
void MessageOutStream::stream(Message::Pointer message, SendPromise::Pointer promise)
{
	boost::asio::dispatch(strand_,
		[this, self = this->shared_from_this(),
		 message = std::move(message), promise = std::move(promise)]() mutable {
...
common::Data MessageOutStream::compoundFrame(FrameType frameType, const common::DataConstBuffer& payloadBuffer)
{
    ...
    const bool isInputEvent = message_->getChannelId() == ChannelId::INPUT &&
                              payloadBuffer.size >= 2 &&
                              payloadBuffer.cdata[0] == 0x80 &&
                              payloadBuffer.cdata[1] == 0x01;

    if(isInputEvent)
    {
        AASDK_LOG(debug) << "[MessageOutStream] input frame before encryption"
                         << " channel=" << static_cast<unsigned int>(message_->getChannelId())
                         << " frameType=" << static_cast<unsigned int>(frameType)
                         << " messageType=" << static_cast<unsigned int>(message_->getType())
                         << " encryptionType=" << static_cast<unsigned int>(message_->getEncryptionType())
                         << " payloadBytes=" << payloadBuffer.size
                         << " payload=" << hexDump(common::Data(payloadBuffer.cdata,
                                                                    payloadBuffer.cdata + payloadBuffer.size));
    }
    ...
    if(isInputEvent)
    {
        AASDK_LOG(debug) << "[MessageOutStream] input frame after encryption"
                         << " frameBytes=" << data.size()
                         << " encryptedPayloadBytes=" << payloadSize
                         << " frame=" << hexDump(data);
    }
