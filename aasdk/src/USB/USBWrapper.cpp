...
{
    auto* handle = new libusb_hotplug_callback_handle();
    const auto result = libusb_hotplug_register_callback(usbContext_, events, flags, vendor_id, product_id, dev_class, cb_fn, user_data, handle);
    ...
    return HotplugCallbackHandle(handle, [this](auto raw_handle) {
