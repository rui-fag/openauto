...
{
    const auto serializedIndication = indication.SerializeAsString();
    AASDK_LOG(debug) << "[InputServiceChannel] INPUT_EVENT_INDICATION"
                     << " timestamp=" << indication.timestamp()
                     << " dispChannel=" << indication.disp_channel()
                     << " touch=" << indication.has_touch_event()
                     << " bytes=" << serializedIndication.size()
                     << " protobuf=" << hexDump(serializedIndication);
    ...
    AASDK_LOG(debug) << "[InputServiceChannel] input payload"
                     << " messageId=8001"
                     << " bytes=" << message->getPayload().size()
                     << " payload=" << hexDump(std::string(message->getPayload().begin(), message->getPayload().end()));
